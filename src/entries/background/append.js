import { createUrl, createRegExpFromString } from "./utils";
...
function backgroundListener(message) {
	...
	let domain = JSON.parse(message.domain);
...
async function appendText(domain, defaultUrl, pages, pageIndex) {
	...
	if (response.status === 200) {
		...
		let innerHtml = (createRegExpFromString(domain.articlePattern)).exec(await response.text())[1];
		if (!innerHtml) return;
...
function getPages(paginationPattern) {
	...
	let regEx = createRegExpFromString(paginationPattern);
	var match;
